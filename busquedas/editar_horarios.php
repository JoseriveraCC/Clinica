...
$horarios_id = $_GET['horarios'];

// Obtener datos actuales
$sql = "SELECT * FROM horarios WHERE horarios = ?";
$stmt = $conexion->prepare($sql);
$stmt->bind_param("i", $horarios_id);
$stmt->execute();
...
<body>
    <form action="actualizar_horarios.php" method="POST">
    <h2>Editar Horario</h2>
    <input type="hidden" name="horarios" value="<?= $horarios['horarios'] ?>">

    <label for="hora_Inicio">Hora de Inicio:</label>
    <input type="time" name="hora_Inicio" id="hora_Inicio" value="<?= $horarios['hora_Inicio'] ?>" required>

    <label for="hora_Fin">Hora de Cierre:</label>
    <input type="time" name="hora_Fin" id="hora_Fin" value="<?= $horarios['hora_Fin'] ?>" required>
